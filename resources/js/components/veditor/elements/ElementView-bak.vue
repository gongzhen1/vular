<template>
  <component v-if="notDeleted" :is="value.name" v-bind="nodProps" :class="classes" :style="styles"
    ...
    @click="click($event)"
    >
    <template v-for="(com,i) in children">
      <element-view :key="i"  :value="com"></element-view>
    </template>
    <element-toolbar v-if="hasToolbar"></element-toolbar>
  </component>
</template>
...
export default {
  ...
  created () {
    console.log(this.inputValue.name, 'created')
  ...
  mounted(){
    console.log(this.inputValue.name,'mounted')
    this.stateChange(this.inputValue)
  },

  destoryed () {
    console.log(this.inputValue.name,'destoryed')
