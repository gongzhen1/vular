...
export default {
  ...
  methods: {
    ...
    addChild(data){
      ...
      if(data.parent._vid === this.inputValue._vid){
        ...
        //if(data.index){
          this.inputValue.children.splice(data.index, 0, data.child)
        //}
        //else{
        //  this.inputValue.children.push(data.child)
        //}
      }      
    },

    insertPlaceholder(data){
      console.log('insertPlaceholder', data)
      if(data.element._vid === this.inputValue._vid){
        data.element.parent = this.inputValue
        //if(data.index){
          console.log('insertPlaceholder', data.index)
          let oldOwner = data.placeholder.parent
          data.placeholder.parent = this.inputValue
          this.inputValue.children.splice(data.index, 0, data.placeholder)

          if(oldOwner) $editorBus.$emit('removePlaceholder',{owner:oldOwner, placeholder:data.placeholder})
        //}
        //else{
        //  this.inputValue.children.push(data.placeholder)
        //}
      }
    },

    removePlaceholder(data){
      if(data.owner._vid === this.inputValue._vid){
        for(var i = 0; i < this.inputValue.children; i++){
          if(this.inputValue.children[i]._vid == data.placeholder._vid){
            this.inputValue.children.splice(i, 1)
            return
          }
        }
  ...
  created () {
    ...
    $editorBus.$on('insertPlaceholder', this.insertPlaceholder)
    $editorBus.$on('removePlaceholder', this.removePlaceholder)
  ...
  destoryed () {
    ...
    $editorBus.$off('insertPlaceholder', this.insertPlaceholder)
    $editorBus.$off('removePlaceholder', this.removePlaceholder)
